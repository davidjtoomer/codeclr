...
parser.add_argument('--data_dir', type=str, default='data', help='The directory name that stores the data generated from download_data.py.')
...
for benchmark in args.benchmark:
    ...
    if not os.path.exists(DATA_DIR):
        logger.error(f'Data directory {DATA_DIR} does not exist. Could not generate statistics for benchmark {benchmark}.')
        continue
    ...
    for directory in tqdm.tqdm(directories, leave=False):
        files = [f for f in os.listdir(os.path.join(DATA_DIR, directory)) if os.path.isfile(os.path.join(DATA_DIR, directory, f))]
        num_files.append(len(files))

        for file in files:
            cass_trees = load_file(os.path.join(DATA_DIR, directory, file))
            num_cass_tree_nodes.append(sum([len(cass_tree.nodes) for cass_tree in cass_trees]))

    logger.info(f'Number of submissions: {sum(num_files)}')

    logger.info(f'Mean number of submissions per problem: {mean(num_files)}')
    logger.info(f'Median number of submissions per problem: {median(num_files)}')
    ...
    logger.info(f'Median number of nodes per CASS: {median(num_cass_tree_nodes)}')
